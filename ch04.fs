...
\
\ When checking a value, the pattern is (assuming the value is
...
\   ?DUP IF ..... THEN 
\ 
\ Truth values were 0 and 1, with the C convention that
\ anything that is not false is true. Modern Forth uses -1 for
\ true but the
\ "if it isn't false it must be true" behavior is still
\ available with caveats.

\ NOT is a poorly defined word in the Forth standard. It can
\ either mean INVERT (just flip the bits) or =0 which will
\ return a proper -1 for true and 0 for false. I think of NOT
\ as =0, but the main lesson is "don't use NOT".

\ The usual explanations of boolean operators instead of
\ addition or subtraction is offered in the text.
...
: card ( n -- , is a person allowed to purchase alcohol? )
   21 < if ." under age"
   else ." drink away" then ;
...
\ 5. Guard our original stars from chapter 1 so that it will
\ not print anything for zero or negative n.

\ Prior definition of stars ... second shadows first. This
\ sequence will get redefinition warnings. This ok.
...
\ print between, not between, or match (only possible when low
\ = high). Use ?RANGE and remember that it is a right open
\ interval.
