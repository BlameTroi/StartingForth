\ zimer-blocks.fth -- A copy of Tom Zimmer's blocks -- T. Brumley.

\ BLOCK.F       Tom's Forth virtual block system.       by Tom Zimmer

\ I'm mining for code and ideas. The original was found at:
\ 
\ https://github.com/quozl/cforth-1/blob/master/src/cforth/block.fth
\
\ 
\ cr .( Loading BLOCK...)
\
\ comment:
\
\ Here is an impementation of a virtual block system.  The
\ constants below, B/BUF, and #BUFFERS control the record
\ or block size, and the number of buffers the system uses.
\ These are defaulted to 1024 byte blocks, and 4 buffers.  A
\ true LRU (least recently used) buffer allocation mechanism
\ is used, implemented as a bubble up buffer stack.  The least
\ recently used buffer is always on the bottom of the stack.
\ As buffers are used or re-used, they are bubbled immediately
\ up to the top of the stack, destined to settle to the bottom
\ of the stack if the same record is not accessed again.
\ 
